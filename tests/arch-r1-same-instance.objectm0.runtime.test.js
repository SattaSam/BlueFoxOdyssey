...
test('ARCH-04: ObjectM0 accepte la collecte de l’instance observée', async () => {
  const { BF, engine, manager, position } = fixture();
  await BF.mount({ engine });

  assert.equal(
    manager.startMission('ARCH-04', { primary: true, autoPrimaryEligible: false }),
    true
  );
  const tree = manager.trees.get('ARCH-04');
  assert.ok(tree);

  const definition = BF.ObjectLibrary.get('relay_block');
  assert.ok(definition);
  const observed = objectValue(definition, 'relay-1', position(1));
  emitStudy(BF, observed, 'ARCH-04', 'ARCH-04:observeComponent', {
    cuoType: 'relay_block'
  });

  const observe = tree.find('ARCH-04:observeComponent');
  const collect = tree.find('ARCH-04:collectComponent');
  assert.equal(observe.progress, 1);
  assert.equal(collect.progress, 0);

  BF.ObjectEvents.emit(BF.ObjectEvents.types.RESOURCE_COLLECTED, observed, {
    mapId: 'map-a',
    missionId: 'ARCH-04',
    missionNodeId: 'ARCH-04:collectComponent',
    cuoType: 'relay_block',
    interactionSource: 'mission'
  });

  assert.equal(collect.progress, 1);
  assert.equal(manager.ensureLifecycle('ARCH-04').status, 'completed');
});

test('ARCH-04: ObjectM0 refuse une autre instance relay_block', async () => {
  const { BF, engine, manager, position } = fixture();
  await BF.mount({ engine });
  ...
  const observed = objectValue(definition, 'relay-1', position(1));
  const other = objectValue(definition, 'relay-2', position(2));
  ...
  BF.ObjectEvents.emit(BF.ObjectEvents.types.RESOURCE_COLLECTED, other, {
    mapId: 'map-a',
    missionId: 'ARCH-04',
    missionNodeId: 'ARCH-04:collectComponent',
    cuoType: 'relay_block',
  ...
  assert.equal(collect.progress, 0);
  assert.notEqual(manager.ensureLifecycle('ARCH-04').status, 'completed');
});
