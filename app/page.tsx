...
function World3D({
  ...
  useEffect(() => {
    ...
    let animationMixer: import("three").AnimationMixer | null = null;
    const animationActions = new Map<string, import("three").AnimationAction>();
    let activeAnimation = "";
    const playAnimation = (name: string, fade = 0.22, once = false) => {
      const next = animationActions.get(name);
      if (!next || activeAnimation === name) return;
      const previous = animationActions.get(activeAnimation);
    ...
    new GLTFLoader().load(
      ...
      (gltf) => {
        ...
        const scale = 2.25 / Math.max(size.y, 0.001);
        model.scale.setScalar(scale);
        const grounded = new THREE.Box3().setFromObject(model);
        const centered = grounded.getCenter(new THREE.Vector3());
        model.position.set(-centered.x, -grounded.min.y, -centered.z);
        foxBaseY = foxFacing.position.y;
        foxFacing.rotation.y = Math.PI;

        animationMixer = new THREE.AnimationMixer(model);
        gltf.animations.forEach((clip) => {
          animationActions.set(clip.name, animationMixer!.clipAction(clip));
        });
        playAnimation(animationActions.has("Idle") ? "Idle" : "Idle_V2", 0);

        foxReady = true;
        onStatus("Je vais commencer par observer les alentours.");
        onSpeak("Je vais d’abord observer cette vallée cristalline.");
        onAction(`BlueFox V0.13 chargé avec ${gltf.animations.length} animations intégrées.`);
      },
      undefined,
      () => onStatus("Le nouveau modèle animé de BlueFox n’a pas pu être chargé."),
    );
    ...
    const animate = () => {
      ...
      if (moving) {
        ...
        foxFacing.position.y = foxBaseY;
        playAnimation("Walk");
      } else if (foxReady) {
        if (!currentObject || actionStartedAt) playAnimation(animationActions.has("Idle") ? "Idle" : "Idle_V2");
        if (currentObject) {
          if (!actionStartedAt) {
            actionStartedAt = performance.now();
            const kind = currentObject.userData.kind as string;
            const harvestAnimation = kind === "crystal" || kind === "fiber"
              ? (animationActions.has("Harvers_Samall") ? "Harvers_Samall" : "Harvest_Medium")
              : animationActions.has("Harvest_Medium") ? "Harvest_Medium" : "Idle";
            playAnimation(harvestAnimation, 0.16, true);
