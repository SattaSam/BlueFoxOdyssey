(function (global) {
  ...
  const PRODUCTION_SPECS = Object.freeze([
    ...
    { id: "DOC-BIO-FLUV-S-001", type: "fluorescent_vegetation", label: "Végétation fluorescente", category: "flora", subtype: "fluorescent_ground_vegetation", size: "S", rarity: "common", biomes: ["forest", "jungle", "swamp", "alien"], scenes: ["Sous-bois fluorescent", "Rive lumineuse"], states: ["dormante", "lumineuse", "inspectée", "récoltée", "repousse"], actions: ["observe", "inspect", "collect", "analyze"], defaultAction: "inspect", acquisitionAction: "collect", observeBeforeAcquire: true, requiresInspectionBeforeCollect: true, afterInspectionAction: "collect", collectable: true, inspectable: true, respawn: 120, inventoryKey: "biocapital", inventoryLabel: "BioCapital", exploitability: "harvestable", family: "flora", resourceFamily: "biocapital", quantity: 2, research: ["botany", "bioluminescence", "energy"], tags: ["resource", "plant", "glowing", "ground_cover"], spawnCost: 1, maxPerZone: 12, minDistance: 0.8, maxSlope: 30, radius: 0.5, volume: "small", curiosity: 0.55, harvest: 0.35, decision: "validated" },
    ...
    { id: "TEC-DRON-L-001", type: "abandoned_drone", label: "Drone abandonné", category: "technology", subtype: "abandoned_exploration_drone", size: "L", rarity: "rare", biomes: ["ruins", "desert", "city", "magnetic", "electrical", "alien"], scenes: ["Épave technologique", "Relais oublié"], states: ["échoué", "inactif", "signal résiduel", "inspecté", "analysé", "récupéré"], actions: ["observe", "inspect", "analyze", "collect"], defaultAction: "inspect", acquisitionAction: "collect", requiresInspectionBeforeCollect: true, afterInspectionAction: "collect", collectable: true, inspectable: true, obstacle: true, respawn: 86400, inventoryKey: "core", inventoryLabel: "Puce de mémoire", quantity: 2, exploitability: "salvageable", family: "technology", resourceFamily: "core", research: ["robotics", "engineering", "ancient-technology"], tags: ["technology", "drone", "wreck", "component", "landmark", "rare"], spawnCost: 9, maxPerZone: 1, minDistance: 12, maxSlope: 18, radius: 1.85, volume: "large", interactionRadius: 1.7, approachDistance: 2.2, curiosity: 1, harvest: 0.72, danger: 0.08, missions: ["technologie perdue", "robotique"], decision: "validated" },
    ...
    { id: "BIO-CARN-L-001", type: "carnivorous_plant", label: "Plante carnivore", category: "flora", subtype: "alien_carnivorous_plant", size: "L", rarity: "uncommon", biomes: ["forest", "jungle", "swamp", "fungal", "alien"], scenes: ["Nid végétal", "Sous-bois prédateur"], states: ["fermée", "en veille", "ouverte", "alerte", "observée", "analysée"], actions: ["observe", "inspect", "analyze", "avoid"], defaultAction: "observe", collectable: false, inspectable: true, obstacle: true, family: "flora", research: ["botany", "toxicology", "adaptation"], tags: ["plant", "carnivorous", "hazard", "living", "resource"], spawnCost: 6, maxPerZone: 2, minDistance: 7, maxSlope: 24, radius: 1.35, volume: "large", curiosity: 0.9, harvest: 0.42, danger: 0.62, missions: ["flore dangereuse", "échantillon biologique"], decision: "validated" },
  ...
  const PRODUCTION_OBJECT_LIBRARY = Object.freeze(Object.fromEntries(PRODUCTION_SPECS.map((spec) => [
    ...
    Object.freeze({
      ...
      interaction: {
        ...
        observeBeforeAcquire: Boolean(spec.observeBeforeAcquire),
        interactionRadius: Number.isFinite(Number(spec.interactionRadius)) ? Number(spec.interactionRadius) : null,
        approachDistance: Number.isFinite(Number(spec.approachDistance)) ? Number(spec.approachDistance) : null,
      ...
      resource: spec.collectable ? { family: spec.resourceFamily, exploitability: spec.exploitability, inventoryKey: spec.inventoryKey, inventoryLabel: spec.inventoryLabel || null, quantity: Math.max(1, Number(spec.quantity) || 1) } : null,
  ...
  BF.ObjectLibrary = Object.freeze({
    ...
    list(filters = {}) {
      ...
      // L'inventaire courant interroge le catalogue sans filtre pour résoudre
      // le nom d'une inventoryKey. Dans ce seul cas, exposer le libellé de la
      // ressource évite de confondre l'objet source avec ce qu'il fournit.
      if (Object.keys(filters).length === 0) {
        return filtered.map((definition) =>
          definition.resource?.inventoryLabel
            ? Object.freeze({
                ...definition,
                label: definition.resource.inventoryLabel
              })
            : definition
        );
      }
